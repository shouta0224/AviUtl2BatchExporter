...
static void AddProjectToListView(const ProjectInfo& pi) {
    ...
    LVITEMW lvi0 = { 0 };
    lvi0.mask = LVIF_TEXT;
    lvi0.iItem = ListView_GetItemCount(g_hListView);
    lvi0.iSubItem = 0;
    lvi0.pszText = const_cast<LPWSTR>(pi.projectPathBuf.data());
...
static INT_PTR CALLBACK BatchRegisterDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: {
        ...
        if (g_hListView) {
            ...
            lvc.pszText = (LPWSTR)col1; lvc.cx = 200; ListView_InsertColumn(g_hListView, 0, &lvc);
            lvc.pszText = (LPWSTR)col2; lvc.cx = 150; ListView_InsertColumn(g_hListView, 1, &lvc);
            lvc.pszText = (LPWSTR)col3; lvc.cx = 150; ListView_InsertColumn(g_hListView, 2, &lvc);
        }
    ...
    case WM_COMMAND: {
        ...
        case IDC_BTN_ADD_PROJECT: {
            ...
            ofn.lpstrFile = szFileMulti; ofn.nMaxFile = MAX_PATH * 10;
            ...
            if (GetOpenFileNameW(&ofn)) {
                ...
                while ((end = file_list.find(L'\0', start)) != std::wstring::npos) {
                    ...
                    if (!current_file.empty()) {
                        ProjectInfo pi; pi.project_path = current_file; pi.output_path = current_output_folder;
                        wchar_t filename_buffer[MAX_PATH] = { 0 };
                        const wchar_t* file_part = PathFindFileNameW(current_file.c_str());
                        if (file_part) {
                            wcscpy_s(filename_buffer, MAX_PATH, file_part); PathRemoveExtensionW(filename_buffer); wcscat_s(filename_buffer, MAX_PATH, L".mp4"); pi.output_filename = filename_buffer;
                        }
                        else { pi.output_filename = L"output.mp4"; }
                        pi.projectPathBuf.assign(pi.project_path.begin(), pi.project_path.end()); pi.projectPathBuf.push_back(L'\0');
                        pi.outputPathBuf.assign(pi.output_path.begin(), pi.output_path.end()); pi.outputPathBuf.push_back(L'\0');
                        pi.outputFilenameBuf.assign(pi.output_filename.begin(), pi.output_filename.end()); pi.outputFilenameBuf.push_back(L'\0');
                        g_registered_projects.push_back(pi); AddProjectToListView(pi);
                    }
                    start = end + 1;
                }
                if (start < file_list.length()) {
                    ...
                    if (!current_file.empty()) {
                        ...
                        wchar_t filename_buffer[MAX_PATH] = { 0 };
                        const wchar_t* file_part = PathFindFileNameW(current_file.c_str());
                        if (file_part) { wcscpy_s(filename_buffer, MAX_PATH, file_part); PathRemoveExtensionW(filename_buffer); wcscat_s(filename_buffer, MAX_PATH, L".mp4"); pi.output_filename = filename_buffer; }
                        else { pi.output_filename = L"output.mp4"; }
                        pi.projectPathBuf.assign(pi.project_path.begin(), pi.project_path.end()); pi.projectPathBuf.push_back(L'\0');
                        pi.outputPathBuf.assign(pi.output_path.begin(), pi.output_path.end()); pi.outputPathBuf.push_back(L'\0');
                        pi.outputFilenameBuf.assign(pi.output_filename.begin(), pi.output_filename.end()); pi.outputFilenameBuf.push_back(L'\0');
        ...
        case IDC_BTN_BROWSE_FOLDER: {
            BROWSEINFO bi = { 0 }; wchar_t folder_path_buffer[MAX_PATH] = { 0 };
            bi.hwndOwner = hDlg; bi.pszDisplayName = folder_path_buffer; bi.lpszTitle = L"Select Output Folder"; bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE;
            LPITEMIDLIST pidl = SHBrowseForFolder(&bi);
            if (pidl != NULL) {
                if (SHGetPathFromIDListW(pidl, folder_path_buffer)) { SetDlgItemTextW(hDlg, IDC_EDIT_OUTPUT_FOLDER, folder_path_buffer); }
                IMalloc* pMalloc = NULL; if (SUCCEEDED(SHGetMalloc(&pMalloc))) { pMalloc->Free(pidl); pMalloc->Release(); }
            }
            break;
        }
        case IDC_BTN_REMOVE_PROJECT: {
            ...
            if (selected_index != -1) {
                if (selected_index < g_registered_projects.size()) { g_registered_projects.erase(g_registered_projects.begin() + selected_index); }
                ListView_DeleteItem(g_hListView, selected_index);
            }
            break;
        }
        case IDC_BTN_CLEAR_PROJECTS: {
            ListView_DeleteAllItems(g_hListView); g_registered_projects.clear();
            break;
        }
        case IDOK: {
            wchar_t output_folder[MAX_PATH]; GetDlgItemTextW(hDlg, IDC_EDIT_OUTPUT_FOLDER, output_folder, MAX_PATH);
            for (auto& pi : g_registered_projects) { pi.output_path = output_folder; }
...
static void ShowBatchRegisterDialog() {
    INT_PTR result = DialogBoxParamW(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_BATCH_REGISTER_DIALOG), NULL, BatchRegisterDialogProc, NULL);
    if (result == IDOK) { MessageBoxW(NULL, L"Batch registration processed.", L"Success", MB_OK); }
    else { MessageBoxW(NULL, L"Batch registration cancelled.", L"Cancelled", MB_OK); }
}
...
extern "C" __declspec(dllexport) OUTPUT_PLUGIN_TABLE* __stdcall GetOutputPluginTable(void) {
    ...
    g_output_table.func_config = [](HWND hwnd, HINSTANCE dll_hinst) -> bool { ShowBatchRegisterDialog(); return true; };
...
bool WINAPI DefineFuncOutput(OUTPUT_INFO* oip) {
    ...
    for (int i = 0; i < total_projects; ++i) {
        ...
        // AviUtl2 の標準出力機能との連携は、具体的なAPI仕様によります。
        // 現状はダミーのエンコード処理を行います。

        // ダミーエンコード: AviUtl2 の標準出力機能を呼び出す想定
        // oip->savefile は現在の出力ファイル名を示しているはずです。
        // バッチ処理では、各プロジェクトの出力ファイル名を個別に設定する必要があります。
        //AviUtl2本体にバッチ処理用の特別なAPIがあるか、またはエンコーダープラグインに
        //プロジェクト情報を渡す別の方法が必要になるかもしれません。

        // 仮のエンコード処理メッセージ
        std::wstring encode_sim_msg = L"Simulating encode for: ";
        encode_sim_msg += pi.project_path;
        // MessageBoxW(g_hBatchDialog, encode_sim_msg.c_str(), L"Encoding", MB_OK); // 多すぎるのでコメントアウト
        ...
        completed_projects++;
        // 進捗表示を更新 (現在の処理中のプロジェクト数 / 総プロジェクト数)
        // 注意: oip->func_rest_time_disp は AviUtl2 のメインウィンドウに進捗を表示するため、
        //     このループ内で頻繁に呼び出すと、AviUtl2 のUIが固まる可能性があります。
        //     適宜、更新頻度を調整するか、プラグインダイアログ内のステータスバーで管理するのが良いでしょう。
        if (i % 5 == 0 || i == total_projects - 1) { // 例: 5フレームごと、または最後のフレームで表示更新
            oip->func_rest_time_disp(i + 1, total_projects);
        }
    }

    // --- 全てのエンコードが完了したら ---
    // TODO: シャットダウン確認ダイアログを表示し、選択に応じて実行する。
    int msgResult = MessageBoxW(g_hBatchDialog,
        (L"Batch encoding finished.\n"
            L"Completed: " + std::to_wstring(completed_projects) + L"/" + std::to_wstring(total_projects) + L"\n"
            L"Do you want to shut down your PC?").c_str(),
        L"Batch Encoding Complete",
        MB_YESNO | MB_ICONQUESTION);

    if (msgResult == IDYES) {
        // --- PCシャットダウン処理 ---
        // ExitWindowsEx(EWX_SHUTDOWN, EWX_SHUTDOWN); // EWX_SHUTDOWN フラグはユーザーログオフ時などに問題を起こす可能性があります。
        // より安全な方法として、shutdownコマンドを実行するなどの方法があります。
        // ただし、シャットダウンはシステム全体に影響を与えるため、十分注意が必要です。
        // 例: system("shutdown -s -t 0"); (これはC++標準ではありませんが、Windows APIで実現可能です)
        // ここでは、シャットダウン処理を実行する代わりにメッセージを表示しておきます。
        MessageBoxW(g_hBatchDialog, L"PC shutdown initiated (simulated).", L"Shutdown", MB_OK);
        // ShellExecute(NULL, L"shutdown", L"/s /t 0", NULL, NULL, SW_HIDE); // 管理者権限が必要な場合あり
    }
    else {
        MessageBoxW(g_hBatchDialog, L"PC shutdown cancelled.", L"Shutdown Cancelled", MB_OK);
    }
